    ...
    while sam:
        ...
        if flag != "4": #we ignore 4
            ...
            if positions != None:
                ...
                if strands != None:
                    barcodes = strands.get(barcode)
                    if barcodes != None:
                        duplicate = True
            if duplicate == False:  #we write only uniq barcodes
                fout.write(sam)
                reads.setdefault(chr, {}).setdefault(read_position, {}).setdefault(strand,{}).setdefault(barcode)
        ...
        for read_position, pos_reads in chr_reads.items():
            for strand, strand_reads in pos_reads.items():
                ...
                if (strand == '+') and (strand_reads.__len__() > 0):
                    fout.write(chr + '\t' + str(start-2) + '\t' + str(start-1) + '\t' + str(strand_reads.__len__()) + '\t' + "" + '\t' + '+' + '\n')
                if (strand == '-') and (strand_reads.__len__() > 0):
                    fout.write(chr + '\t' + str(end) + '\t' + str(end+1) + '\t' + str(strand_reads.__len__()) + '\t' + "" + '\t' + '-' + '\n')
    fout.close()
...
#PATH = "/home/nebo/UCL/2013.10.30-reMapping-iCount-Jernej/all_SmB_Cal51_Mphase_hg19/"
#sam_file_to_bed(PATH + "20130808_LUc31_1_hg19_ensembl59_S_notmapped.sam",  PATH + "20130808_LUc31_1_rb.fasta", PATH + "20130808_LUc31_1_hg19_ensembl59_S_notmapped-collapsed.sam", PATH + "20130808_LUc31_1_hg19_ensembl59_S_notmapped-collapsed.bed")
